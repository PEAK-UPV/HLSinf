...
module MUL_BATCH #(
  ...
  parameter REP_INFO              = GROUP_SIZE + 1,                  // number of bits for repetition detector
  localparam MUL_OUTPUT            = 2 * DATA_WIDTH + REP_INFO,               // MUL module output number of bits
  localparam OUTPUT_WIDTH          = GROUP_SIZE * 2 * DATA_WIDTH              // MUL_BATCH output number of bits
)(
  input clk,
  input rst,
  ...
  input [DATA_WIDTH - 1 : 0]               act_data_in,             // ACTIVATION interface:: activations data
  input [REP_INFO - 1 : 0]                 act_rdata_in,            // ACTIVATION interface: activations rdata
...
MUL_RD #(
  ...
  .act_data_in            ( {act_rdata_in,act_data_in} ), //TOP  to MUL
  .act_valid_in           ( act_valid_in         ), //TOP  to MUL  
  .act_avail_out          ( act_avail_out        ), //MUL  to TOP 
