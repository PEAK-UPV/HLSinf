...
HLS_EXTERNAL void lib_conv2D( 
      ...
      #ifdef HLS_DEBUG
      ,
      unsigned long                     dbg_streams_mask, // mask for debug, enable/disable write to dbg streams 
  ...
  #ifdef HLS_DEBUG
  unsigned long dbg_loop_stream_mask;
  unsigned long dbg_loop_stream_o_iter_monitor;

  dbg_loop_stream_mask = dbg_streams_mask;
  dbg_loop_stream_o_iter_monitor = dbg_streams_mask & HLS_DBG_ENABLE_STREAM_hls_o_iter_monitor_MASK;
  ...
  for (int o_iter = 0; o_iter<O_ITER; o_iter++) {
    ...
    int c_out_pixels_read = 0; // current value of pixels read  to be written to global memory

    //unsigned long int  hld_dbg_buffer_input_counter;


    // we first need to launch the tasks
    ihc::launch<input_buffer>( o_iter, dbg_loop_stream_mask, read_pixels_total, write_to_input_buffer, read_from_input_buffer);
    ...
    ihc::launch<padding>(o_iter, dbg_loop_stream_mask, H, W, I_ITER, enable_upper_padding, enable_lower_padding);   // padding
    ihc::launch<cvt>(o_iter, dbg_loop_stream_mask, H, W, I_ITER);
    ihc::launch<mul>(o_iter, dbg_loop_stream_mask, H, W, I_ITER);
    ihc::launch<add>(o_iter, dbg_loop_stream_mask, H, W, I_ITER);
    ...
    { 
      ...
      for (int i = 0; i < read_pixels_total; i++) {
        ...
        {
          if ((unsigned long)dbg_loop_stream_mask & HLS_DBG_ENABLE_STREAM_data_in_MASK) {
            for (int j = 0; j < CPI; j++) {
        ...
        // check for data ready to be writen to global memory -> in order to avoid channel congestion
        {
          if (c_out_pixels_read < write_pixels) {
            bool read_success = false;            
            write_block_t bx = out_pooling.tryRead(read_success);
            if (read_success) {
              #ifdef HLS_DEBUG
              my_flt_dout = my_flt_dout + bx.pixel[0] + bx.pixel[1] + bx.pixel[2] + bx.pixel[3]; 
              #endif
              for(int j = 0; j < CPO; j++) {
                ptr_out[o_iter_write_offset + c_out_pixels_read].pixel[j] = bx.pixel[j];
              }
              #ifdef HLS_DEBUG
              dbg_elements_per_iter_data_out[o_iter] = dbg_elements_per_iter_data_out[o_iter] + 1;
              #endif

              c_out_pixels_read = c_out_pixels_read + 1;
            }
          }
        }
      }
    ...
    // write remaining output pixels to memory
    //{
      write_data_channels_loop_pixels:
      for (int i = c_out_pixels_read; i < write_pixels; i++) {
    ...
    dbg_ptr_ul[HLS_DBG_VALUES_read_from_bias_IND]    = dbg_elements_per_iter_bias[dbg_loop_stream_o_iter_monitor];
    dbg_ptr_ul[HLS_DBG_VALUES_read_from_kernel_IND]  = dbg_elements_per_iter_kernel[dbg_loop_stream_o_iter_monitor];
    dbg_ptr_ul[HLS_DBG_VALUES_read_from_data_in_IND] = dbg_elements_per_iter_data_in[dbg_loop_stream_o_iter_monitor];
    dbg_ptr_ul[HLS_DBG_VALUES_write_to_bias_stream_IND]   =  dbg_elements_per_iter_bias[dbg_loop_stream_o_iter_monitor];
    dbg_ptr_ul[HLS_DBG_VALUES_write_to_kernel_stream_IND] = dbg_elements_per_iter_kernel[dbg_loop_stream_o_iter_monitor];


    dbg_ptr_ul[HLS_DBG_VALUES_write_to_out_read_data_stream_IND] = dbg_elements_per_iter_data_in[dbg_loop_stream_o_iter_monitor];
    dbg_ptr_ul[HLS_DBG_VALUES_write_to_out_read_data_from_input_buffer_stream_IND] = dbg_elements_per_iter_data_input_buffer[dbg_loop_stream_o_iter_monitor];
    //dbg_ptr_ul[HLS_DBG_VALUES_write_to_out_read_data_from_input_buffer_stream_IND] = hld_dbg_buffer_input_counter;
    
    dbg_ptr_ul[HLS_DBG_VALUES_write_to_pad_cvt_stream_IND]  = dbg_elements_per_iter_data_dc_pad_out[dbg_loop_stream_o_iter_monitor];
    dbg_ptr_ul[HLS_DBG_VALUES_write_to_cvt_mul_stream_IND]  = dbg_elements_per_iter_data_dc_cvt_out[dbg_loop_stream_o_iter_monitor];
    dbg_ptr_ul[HLS_DBG_VALUES_write_to_mul_add_stream_IND]  = dbg_elements_per_iter_data_dc_mul_out[dbg_loop_stream_o_iter_monitor];
    dbg_ptr_ul[HLS_DBG_VALUES_write_to_out_conv_stream_IND] = dbg_elements_per_iter_data_dc_add_out[dbg_loop_stream_o_iter_monitor];
    //dbg_ptr_ul[] = dbg_elements_per_iter_data_dc_direcconv_out[dbg_loop_stream_o_iter_monitor];
    //dbg_ptr_ul[] = dbg_elements_per_iter_data_out[dbg_loop_stream_o_iter_monitor];
    dbg_ptr_ul[HLS_DBG_VALUES_write_to_out_relu_stream_IND] = dbg_elements_per_iter_data_relu_out[dbg_loop_stream_o_iter_monitor];
    dbg_ptr_ul[HLS_DBG_VALUES_write_to_stream_pool_stream_IND] = dbg_elements_per_iter_data_pool_cvt_out[dbg_loop_stream_o_iter_monitor];
    dbg_ptr_ul[HLS_DBG_VALUES_write_to_out_pooling_stream_IND] = dbg_elements_per_iter_data_pool_pooling_out[dbg_loop_stream_o_iter_monitor];
   
    dbg_ptr_ul[HLS_DBG_VALUES_write_to_data_out_IND] =dbg_elements_per_iter_data_out[dbg_loop_stream_o_iter_monitor];   

    dbg_ptr_ul[HLS_DBG_VALUES_write_to_out_conv_sbs_stream_IND] =  dbg_elements_per_iter_data_dc_cvt_sbs_out[dbg_loop_stream_o_iter_monitor]; 

    my_flt_bias = dbg_flt_bias_sum[dbg_loop_stream_o_iter_monitor];
    my_flt_krnl = dbg_flt_krnl_sum[dbg_loop_stream_o_iter_monitor];
    my_flt_din  = dbg_flt_din_sum[dbg_loop_stream_o_iter_monitor];
    ...
    unsigned long dbg_words_write_to_out_read_data_stream = 0;
    unsigned long dbg_words_write_to_out_read_data_from_input_buffer_stream = 0;
    unsigned long dbg_words_write_to_pad_cvt_stream = 0;
    unsigned long dbg_words_write_to_cvt_mul_stream = 0;
    unsigned long dbg_words_write_to_mul_add_stream = 0;
    unsigned long dbg_words_write_to_out_conv_stream = 0;
    unsigned long dbg_words_write_to_out_relu_stream = 0;
    unsigned long dbg_words_write_to_stream_pool_stream = 0;
    unsigned long dbg_words_write_to_out_pooling_stream = 0;
    unsigned long dbg_words_write_to_data_out = 0;

    for (int i = 0; i < O_ITER; i++) {
      dbg_words_write_to_out_read_data_stream   += dbg_elements_per_iter_data_in[i];
      dbg_words_write_to_out_read_data_from_input_buffer_stream += dbg_elements_per_iter_data_input_buffer[i];
      dbg_words_write_to_pad_cvt_stream         += dbg_elements_per_iter_data_dc_pad_out[i];
      dbg_words_write_to_cvt_mul_stream         += dbg_elements_per_iter_data_dc_cvt_out[i];
      dbg_words_write_to_mul_add_stream         += dbg_elements_per_iter_data_dc_mul_out[i];
      dbg_words_write_to_out_conv_stream        += dbg_elements_per_iter_data_dc_add_out[i];
      dbg_words_write_to_out_relu_stream        += dbg_elements_per_iter_data_relu_out[i];
      dbg_words_write_to_stream_pool_stream     += dbg_elements_per_iter_data_pool_cvt_out[i];
      dbg_words_write_to_out_pooling_stream     += dbg_elements_per_iter_data_pool_pooling_out[i];
      dbg_words_write_to_data_out               += dbg_elements_per_iter_data_out[i];
    }

    dbg_ptr_ul[HLS_DBG_VALUES_words_write_to_out_read_data_stream_IND] = dbg_words_write_to_out_read_data_stream;
    dbg_ptr_ul[HLS_DBG_VALUES_words_write_to_out_read_data_from_input_buffer_stream_IND] = dbg_words_write_to_out_read_data_from_input_buffer_stream;
    dbg_ptr_ul[HLS_DBG_VALUES_words_write_to_pad_cvt_stream_IND]     = dbg_words_write_to_pad_cvt_stream;
    dbg_ptr_ul[HLS_DBG_VALUES_words_write_to_cvt_mul_stream_IND]     = dbg_words_write_to_cvt_mul_stream;
    dbg_ptr_ul[HLS_DBG_VALUES_words_write_to_mul_add_stream_IND]     = dbg_words_write_to_mul_add_stream;
    dbg_ptr_ul[HLS_DBG_VALUES_words_write_to_out_conv_stream_IND]    = dbg_words_write_to_out_conv_stream;
    dbg_ptr_ul[HLS_DBG_VALUES_words_write_to_out_relu_stream_IND]    = dbg_words_write_to_out_relu_stream;
    dbg_ptr_ul[HLS_DBG_VALUES_words_write_to_stream_pool_stream_IND] = dbg_words_write_to_stream_pool_stream;
    dbg_ptr_ul[HLS_DBG_VALUES_words_write_to_out_pooling_stream_IND] = dbg_words_write_to_out_pooling_stream;
    dbg_ptr_ul[HLS_DBG_VALUES_words_write_to_data_out_IND]           = dbg_words_write_to_data_out;
    ...
    if ((unsigned long)dbg_loop_stream_mask & HLS_DBG_ENABLE_STREAM_input_buffer_MASK)
    {
      ...
      for (int i = 0; i < dbg_elements_per_iter_data_input_buffer[dbg_loop_stream_o_iter_monitor]; i++) {
        pixel_in_t px_loop;
    ...
    if ((unsigned long)dbg_loop_stream_mask & HLS_DBG_ENABLE_STREAM_dc_pad_out_MASK)
    {
      unsigned long bytes_to_copy = sizeof(read_block_t);
      //int elements_to_copy =  I_ITER * (H + 2) * (W + 2); // value from padding.cp
      int elements_to_copy =  dbg_elements_per_iter_data_dc_pad_out[dbg_loop_stream_o_iter_monitor];
    ...
    if ((unsigned long)dbg_loop_stream_mask & HLS_DBG_ENABLE_STREAM_dc_cvt_out_MASK)
    {
      ...
      int elements_to_copy = dbg_elements_per_iter_data_dc_cvt_out[dbg_loop_stream_o_iter_monitor];
    ...
    if ((unsigned long)dbg_loop_stream_mask & HLS_DBG_ENABLE_STREAM_dc_cvt_sbs_control_out_MASK) 
    {
      int elements_to_copy = dbg_elements_per_iter_data_dc_cvt_sbs_out[dbg_loop_stream_o_iter_monitor];
      for (int i = 0; i < elements_to_copy; i++) {
    ...
    if ((unsigned long)dbg_loop_stream_mask & HLS_DBG_ENABLE_STREAM_dc_mul_out_MASK)
    {
      ...
      int elements_to_copy = dbg_elements_per_iter_data_dc_mul_out[dbg_loop_stream_o_iter_monitor];
      for (int i = 0; i < elements_to_copy; i++) {
        pixel_out_t px_loop;
    ...
    if ((unsigned long)dbg_loop_stream_mask & HLS_DBG_ENABLE_STREAM_data_directconv_out_MASK)
    {
      ...
      for (int i = 0; i < dbg_elements_per_iter_data_out[dbg_loop_stream_o_iter_monitor]; i++) {
        pixel_out_t px_loop;
