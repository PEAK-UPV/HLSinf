...
//
void read_bias(int offset_bias, struct pixel_out_t *b_ptr, ihc::stream<struct pixel_out_t> &out) {
...
// kernels are sent in frame structures (3x3 grid)
//
void dws_read_dw_kernel(int I_ITER, int o_iter, data_type *k_dw_ptr, ihc::stream<kernel_dw_t> &k_dw_out){
...
void dws_read_pw_kernel(int I_ITER, int O, int o_iter, read_kernel_pw_t *k_pw_ptr, ihc::stream<kernel_pw_t> &k_pw_out){
...
//
void read_data_channels(int H, int W, int rows, int I_ITER, read_block_t *ptr, int offset, int num_extra_rows, int channel_blocks, ihc::stream<read_block_t> out[CPI], int I, int enable) {
