...
//
static void cvt_winograd(int H, int W, int I_ITER, ihc::stream<pixel_in_t> &in, ihc::stream<frame_d> &out) {
...
//
static void frameConvert(int H, int W, int I_ITER, ihc::stream<frame_d> &d_in, ihc::stream<frame_d_2> &out){
...
//   out   : output stream
//
static void mulData(int H, int W, int I_ITER, ihc::stream<frame_d_2> &d_in, ihc::stream<frame_d_2> &out) {
...
//   out   : output stream
//
static void mulKernels(int I_ITER, ihc::stream<kernel_t> &k_in, ihc::stream<frame_d> &out) {
...
//   out   : output stream
//
static void mulWise(int H, int W, int I_ITER, hls::stream<frame_d_2> &d_in, ihc::stream<frame_d> &k_in, ihc::stream<frame_d> &out) {
...
//   out   : output stream
//
static void mult_A_AT(int H, int W, int I_ITER, ihc::stream<frame_d> &d_in, ihc::stream<frame_winograd> &out) {
...
//   out   : output stream
//
static void add_winograd(int H, int W, int I_ITER, hls::stream<pixel_out_t> &b_in, ihc::stream<frame_winograd> &in, ihc::stream<pixel_out_t> &out) {
...
//
void winograd_conv(int H, int W, int I_ITER, int enable_upper_padding, int enable_lower_padding, ihc::stream<pixel_in_t> &in, ihc::stream<kernel_t> &k_in, ihc::stream<pixel_out_t> &b_in, ihc::stream<pixel_out_t> &out) {

	// streams
	ihc::stream<pixel_in_t>      str_pad_cvt;  	 	// padding 	-> 	cvt
	ihc::stream<frame_d>   	    cvt_frameConvert; 	// cvt 		-> 	frameConvert
	ihc::stream<frame_d_2>       str_cvt_mul_cTc;    // cvt 		-> 	mulData
	ihc::stream<frame_d>         kernels_multWise;  	// mulKernels -> mulWise
	ihc::stream<frame_d_2>       mult_data_res;  	// mulData 	-> 	mult wise
	ihc::stream<frame_d>         mult_wise_res;  	// mulWise 	-> 	mult_A_AT
	ihc::stream<frame_winograd> 	str_mul_add;  		// mult_A_AT -> add_winograd
