...
#ifdef USE_UPSIZE
void upsize(int enable, int H, int W, int O_ITER, hls::stream<dout_st> &in, hls::stream<dout_st> &out) {
  dout_st buff[WMAX];

  int iterations = H * W * O_ITER;
  ...
  if (enable) {
    ...
    for (int i = 0; i < iterations; i++) {
      DO_PRAGMA(HLS loop_tripcount min=1 max=H_REFERENCE*W_REFERENCE*(OMAX/CPO))
      dout_st px;
      px = in.read();
	  out << px;
    }
...
extern "C" {
void k_conv2D(read_block_t *ptr_data, hls::stream<read_block_t> &input_stream, write_block_t *ptr_data_add, int H, int W, int HO, int WO, int rows, int PT, int PB, int PL, int PR,
		    int SH, int SW, int I, int O, int I_ITER, int o_iter_first, int o_iter_last, int O_ITER, int enable_instream, int enable_outstream,
            int enable_relu, int enable_stm, float relu_factor,int enable_batch_norm,
            w_t *ptr_kernel,
            b_st *ptr_bias, bnp_st *b_ptr, write_block_t *ptr_out, hls::stream<write_block_t> &output_stream, int read_offset, int write_offset, int enable_maxpooling, int enable_avgpooling,
	...
	DO_PRAGMA(HLS INTERFACE m_axi port=ptr_kernel    depth=KERNEL_PORT_DEPTH    max_widen_bitwidth=512	num_read_outstanding=16   max_read_burst_length=256  num_write_outstanding=1 max_write_burst_length=2	offset=slave bundle=gmem1)
	DO_PRAGMA(HLS INTERFACE m_axi port=ptr_data      depth=DATA_IN_PORT_DEPTH   max_widen_bitwidth=512	num_read_outstanding=16   max_read_burst_length=256	 num_write_outstanding=1 max_write_burst_length=2	offset=slave bundle=gmem)
	DO_PRAGMA(HLS INTERFACE m_axi port=ptr_data_add  depth=DATA_IN_PORT_DEPTH   max_widen_bitwidth=512	num_read_outstanding=16   max_read_burst_length=256  num_write_outstanding=1 max_write_burst_length=2	offset=slave bundle=gmem5)
	DO_PRAGMA(HLS INTERFACE m_axi port=b_ptr         depth=BATCH_MORM_VAL_DEPTH max_widen_bitwidth=512	num_read_outstanding=16   max_read_burst_length=256  num_write_outstanding=1 max_write_burst_length=2	offset=slave bundle=gmem6)
    DO_PRAGMA(HLS INTERFACE m_axi port=ptr_bias      depth=BIAS_PORT_DEPTH      max_widen_bitwidth=512	num_read_outstanding=16   max_read_burst_length=256  num_write_outstanding=1 max_write_burst_length=2	offset=slave bundle=gmem2)
	DO_PRAGMA(HLS INTERFACE m_axi port=ptr_out       depth=DATA_OUT_PORT_DEPTH  max_widen_bitwidth=512	num_write_outstanding=16  max_write_burst_length=256 num_read_outstanding=1  max_read_burst_length=2	offset=slave bundle=gmem3)

    #pragma HLS INTERFACE axis port=input_stream depth=STREAMS_DEPTH
    #pragma HLS INTERFACE axis port=output_stream depth=STREAMS_DEPTH
    ...
    #pragma HLS INTERFACE mode=s_axilite port=enable_instream bundle=control
    #pragma HLS INTERFACE mode=s_axilite port=enable_outstream bundle=control
	...
	DO_PRAGMA(HLS stable variable=PT)
	DO_PRAGMA(HLS stable variable=PB)
	DO_PRAGMA(HLS stable variable=PL)
	DO_PRAGMA(HLS stable variable=PR)
	...
	int read_add_offset   = write_offset + (offset_read_add_group_cpo * o_iter_first);

	/*
	 * Variables that are passed as arguments to functions inside a dataflow region must be inside the region as well
	 * in order for the tools to correctly implement it
	 */
	#pragma HLS dataflow
	...
	read_input(rows, W, offset_read_data_channel, I_ITER, enable_instream, ptr_data, input_stream, out_read_data);
	...
	read_kernel(I, I_ITER, O_ITER, o_iter_first, ptr_kernel, out_read_kernel);
	kernel_buffer(num_output_conv_pixels, I_ITER, O_ITER, out_read_kernel, out_filters_buffer);
	read_input_add(enable_add, read_pixels_add, O_ITER, read_add_offset, ptr_data_add, out_read_data_add);
	read_batch_norm(enable_batch_norm, offset_bias, b_ptr, out_read_batch_norm);

	//--------------------------------------------------------------------------------------------------------------------------------------------------------------------
	// direct convolution
	direct_conv(rows, W, PT, PB, PL, PR, SH, SW, num_output_conv_pixels, I_ITER, O_ITER, out_read_data, out_filters_buffer, out_bias_buffer, out_conv);

	//--------------------------------------------------------------------------------------------------------------------------------------------------------------------
	// Relu, Clipping, shift, pooling, batch normalization, add, and upsize
	relu(enable_relu, enable_clipping, enable_shift, relu_factor, min_clip, max_clip, dir_shift, pos_shift, num_output_conv_pixels, O_ITER, out_conv, out_relu);
	//stm(enable_stm, num_output_conv_pixels, out_relu, out_stm);
	pooling(HI_pooling, WI_pooling, O_ITER, enable_maxpooling, enable_avgpooling, out_relu, out_pooling);
	batch_norm(enable_batch_norm, num_bn_pixels, O_ITER, out_pooling, out_read_batch_norm, out_batch_norm);
	add_data(enable_add, num_add_pixels, O_ITER, out_read_data_add, out_batch_norm, out_add);
	upsize(enable_upsize, write_rows, write_cols, O_ITER, out_add, out_write);

	//--------------------------------------------------------------------------------------------------------------------------------------------------------------------
	// write to memory
	write_data(write_pixels, O_ITER, o_iter_write_offset, enable_outstream, ptr_out, output_stream, out_write);
}
} // extern "C"
