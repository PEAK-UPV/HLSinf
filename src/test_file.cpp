...
int read_test_file(int *enable, int *cpu) {
 int n = fscanf(fp, "ENABLE %d CPU %d %dx%dx%dx%d EUP %d ELP %d RELU %d STM %d MAXPOOL %d AVGPOOL %d SHIFT %d DIRECTION_SHIFT %d POS_SHIFT %d CLIP %d MINCLIP %d MAXCLIP %d\n",
            enable, cpu, &H, &W, &I, &O, &enable_upper_padding, &enable_lower_padding, &enable_relu, &enable_stm, &enable_maxpooling, &enable_avgpooling, &enable_shift, &dir_shift, &pos_shift,
