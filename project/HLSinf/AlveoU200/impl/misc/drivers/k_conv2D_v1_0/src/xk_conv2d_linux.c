...
typedef struct {
    ...
    XK_conv2d_uio_map maps[ MAX_UIO_MAPS ];
} XK_conv2d_uio_info;

/***************** Variable Definitions **************************************/
static XK_conv2d_uio_info uio_info;
...
static int uio_info_read_name(XK_conv2d_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XK_conv2d_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XK_conv2d_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XK_conv2d_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XK_conv2d_Initialize(XK_conv2d *InstancePtr, const char* InstanceName) {
	XK_conv2d_uio_info *InfoPtr = &uio_info;
