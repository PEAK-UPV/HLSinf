...
//int output_data_address(int o, int h, int w) { // obsolete function to delete
//  printf("output_data_address: obsolete function\n");
//	int go = o / CPO;
//	int oo = o % CPO;
...
void cpu_conv2D() {
  ...
  for (int i=0; i<size_out; i++) out_conv_cpu[i] = (data_type)0;
  ...
  for (int cout=0; cout<O_output; cout++) {
    ...
    printf("Channel %d\n", cout);
    printf("WO    ");
    for (int w=0; w<WO; w++) printf("   %8d", w);
    printf("\n");
    ...
    for (int cout=0; cout<O_output; cout++) {
      for (int h=0; h<HO; h++) {
        for (int w=0; w<WO; w++) {
          ...
          if (float(out_conv_cpu[addr_o]) < float(0)) out_relu_cpu[addr_o] = (float)0.f ; else out_relu_cpu[addr_o] = out_conv_cpu[addr_o];
          #ifdef DEBUG_RELU
    ...
    for (int o=0; o<O_output; o++) {
      for (int h=0; h<HO; h=h+2) {
    	for (int w=0; w<WO; w=w+2) {
    	  ...
    	  data_type max_v = (data_type)(-9999999);//-(data_type)0xFFFFFFFFF;
      ...
      for (int h=0; h<HO_final; h++) {
        for (int w=0; w<WO_final; w++) {
  	      int addr_o = output_data_address(cout, h, w, HO_final, WO_final);
				  ...
				  bn_t std, xn;
				  std = sqrtf((float)batch_norm_values[(index*4)+3] + BN_EPS);
